...
app_gatt_connect_handler(wiced_bt_gatt_connection_status_t *p_conn_status)
{
    ...
    else
    {
         ...
         * will be off
         */
        app_xensiv_sensor_shield_sensorenable_client_char_config[0] = 0;
                                               ...
                                               uint16_t len)
{
    ...
    switch (attr_handle)
    {
        case HDLC_XENSIV_SENSOR_SHIELD_SENSORENABLE_VALUE:
            if (app_xensiv_sensor_shield_sensorenable_len >= len)
            {
                /* This case will handle changing screens on mobile app
                 * We will end current sensor task and start reading
                 * from new sensor
                 * */

                /* Value fits within the supplied buffer; copy over the value */
                memcpy(app_xensiv_sensor_shield_sensorenable,
                       p_val,
                       len);

                /* Callback to alert console task new sensor has been enabled */
                console_task_ble_app_callback();
        ...
        case HDLD_XENSIV_SENSOR_SHIELD_PASCO2_CLIENT_CHAR_CONFIG:
        ...
        case HDLD_XENSIV_SENSOR_SHIELD_DPS368_CLIENT_CHAR_CONFIG:
...
/*
 ...
 Function Description:
 @brief  The function is invoked by application to notify client of characteristic change.

 @param attr_handle  GATT attribute handle
 @param p_val        Pointer to Bluetooth LE GATT notify request value
 @param len          length of GATT notify request

 @return wiced_bt_gatt_status_t  Bluetooth LE GATT status
 */
wiced_bt_gatt_status_t app_bt_gatt_notify(uint16_t attr_handle,
                                                          uint8_t  *p_val,
                                          uint16_t len)
{
    wiced_bt_gatt_status_t gatt_status = WICED_BT_GATT_INVALID_HANDLE;

    uint8_t client_config;

    /* Check for a matching handle entry */
    switch (attr_handle)
    {
        case HDLC_XENSIV_SENSOR_SHIELD_BGT60LTR11_VALUE:
            client_config = app_xensiv_sensor_shield_bgt60ltr11_client_char_config[0];
            break;

        case HDLC_XENSIV_SENSOR_SHIELD_SHT35_VALUE:
            client_config = app_xensiv_sensor_shield_sht35_client_char_config[0];
            break;

        case HDLC_XENSIV_SENSOR_SHIELD_BMI270_VALUE:
            client_config = app_xensiv_sensor_shield_bmi270_client_char_config[0];
            break;

        case HDLC_XENSIV_SENSOR_SHIELD_BMM350_VALUE:
            client_config = app_xensiv_sensor_shield_bmm350_client_char_config[0];
            break;

        case HDLC_XENSIV_SENSOR_SHIELD_DPS368_VALUE:
            client_config = app_xensiv_sensor_shield_dps368_client_char_config[0];
            break;

        case HDLC_XENSIV_SENSOR_SHIELD_PASCO2_VALUE:
            client_config = app_xensiv_sensor_shield_pasco2_client_char_config[0];
            break;
    ...
    if (IS_NOTIFIABLE(app_bt_conn_id, client_config) == 0)
    {
        if (!app_bt_conn_id)
        {
            printf("This device is not connected to a central device\r\n");

            gatt_status = WICED_BT_GATT_INSUF_AUTHENTICATION;
        }
        else
        {
            ...
            gatt_status = WICED_BT_GATT_WRITE_REQ_REJECTED;
        }
    }
    else
    {
